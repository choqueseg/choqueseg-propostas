...
function paginasNecessarias(tema: TemaSeguranca, quantidadeItens: number = 0) {
...
const PreviewSeguranca = forwardRef<HTMLDivElement, Props>(
  function PreviewSeguranca({ dados }, ref) {
    const dadosSeguros = (dados ?? {}) as Partial<DadosPreviewSeguranca>;
    const itensSeguros: ItemSegurancaPreview[] = Array.isArray(dadosSeguros.itens)
      ? dadosSeguros.itens.filter(Boolean) as ItemSegurancaPreview[]
      : [];
    const temaPDF: "claro" | "escuro" = dadosSeguros.temaPDF === "escuro" ? "escuro" : "claro";
    const temaPrincipal = detectarTemaPrincipal(itensSeguros);
    const totalPaginas = paginasNecessarias(temaPrincipal, itensSeguros.length);
    const beneficio = beneficioTema(temaPrincipal);
    const dadosNormalizados: DadosPreviewSeguranca = {
      nome: String(dadosSeguros.nome ?? ""),
      telefone: String(dadosSeguros.telefone ?? ""),
      cidade: String(dadosSeguros.cidade ?? ""),
      endereco: String(dadosSeguros.endereco ?? ""),
      observacoes: String(dadosSeguros.observacoes ?? ""),
      itens: itensSeguros,
      subtotal: Number(dadosSeguros.subtotal ?? 0),
      desconto: Number(dadosSeguros.desconto ?? 0),
      total: Number(dadosSeguros.total ?? 0),
      parcelasCartao: Number(dadosSeguros.parcelasCartao ?? 0),
      totalCartao: Number(dadosSeguros.totalCartao ?? 0),
      parcelaCartao: Number(dadosSeguros.parcelaCartao ?? 0),
      temaPDF,
      enderecoEmpresa: String(dadosSeguros.enderecoEmpresa ?? ""),
    };

    const mensagemFechamento = [
      "Olá, CHOQUESEG! Analisei minha proposta de Segurança Eletrônica e quero fechar o serviço.",
      "",
      `Cliente: ${(dadosSeguros.nome ?? "") || "Não informado"}`,
      `Cidade: ${(dadosSeguros.cidade ?? "") || "Não informada"}`,
      `Telefone: ${(dadosSeguros.telefone ?? "") || "Não informado"}`,
      `Valor: ${moeda(Number(dadosSeguros.total ?? 0))}`,
      "",
    ...
    if (totalPaginas === 1) {
      return (
        <div ref={ref} className="grid items-start gap-5">
          <Pagina numeroPagina={1} totalPaginas={1} tema={temaPDF}>
            <Cabecalho tema={temaPrincipal} frase={fraseTema(temaPrincipal)} />
            <DadosCliente nome={(dadosSeguros.nome ?? "")} cidade={(dadosSeguros.cidade ?? "")} data={dataHoje()} tema={temaPDF} />

            <TituloSecao pequeno="Solução completa CHOQUESEG" titulo="O que está incluso na sua instalação" />

            <div className="mt-3 grid grid-cols-2 gap-3">
              {itensSeguros.map((item) => (
                <CardItem key={item.id} item={item} temaPDF={temaPDF} />
              ))}
            </div>

            <Beneficio titulo={beneficio.titulo} texto={beneficio.texto} tema={temaPrincipal} />

            <ResumoFinanceiro dados={dadosNormalizados} linkFechamento={linkFechamento} compacto />

            <Rodape tema={temaPDF} enderecoEmpresa={(dadosSeguros.enderecoEmpresa ?? "")} />
          </Pagina>
        </div>
      );
    }
    ...
    return (
      <div ref={ref} className="grid items-start gap-5">
        <Pagina numeroPagina={1} totalPaginas={totalPaginas} tema={temaPDF}>
          <Cabecalho tema={temaPrincipal} frase={fraseTema(temaPrincipal)} />
          <DadosCliente nome={(dadosSeguros.nome ?? "")} cidade={(dadosSeguros.cidade ?? "")} data={dataHoje()} tema={temaPDF} />
          ...
          <div className="mt-3 grid grid-cols-2 gap-3">
            {itensSeguros.slice(0, 8).map((item) => (
              <CardItem key={item.id} item={item} temaPDF={temaPDF} />
            ))}
          </div>

          {itensSeguros.length > 8 && (
            <p className="mt-3 text-center text-[12px] font-black text-zinc-500">
              + {itensSeguros.length - 8} item(ns) detalhado(s) no fechamento da proposta.
            </p>
          )}
        ...
        {totalPaginas >= 2 && (
          <Pagina numeroPagina={2} totalPaginas={totalPaginas} tema={temaPDF}>
            ...
            <div className="mt-4 grid grid-cols-2 gap-3">
              <Etapa numero="1" titulo="Levantamento técnico" texto="Análise do local e definição do melhor posicionamento e solução." tema={temaPDF} />
              <Etapa numero="2" titulo="Conferência do escopo" texto="Validação dos equipamentos, quantidades e materiais previstos." tema={temaPDF} />
              <Etapa numero="3" titulo="Instalação" texto="Execução profissional com organização, acabamento e segurança." tema={temaPDF} />
              <Etapa numero="4" titulo="Configuração e testes" texto="Ajustes, testes de funcionamento e acesso pelo celular quando aplicável." tema={temaPDF} />
              <Etapa numero="5" titulo="Orientação ao cliente" texto="Explicação de uso, operação e cuidados com o sistema instalado." tema={temaPDF} />
              <Etapa numero="6" titulo="Suporte pós-venda" texto="Acompanhamento da equipe CHOQUESEG após a entrega." tema={temaPDF} />
            </div>

            <div className={`mt-4 rounded-2xl border border-yellow-400 p-5 ${
              temaPDF === "escuro" ? "bg-black text-white" : "bg-yellow-50 text-zinc-950"
            }`}>
            ...
            {totalPaginas === 2 ? (
              <ResumoFinanceiro dados={dadosNormalizados} linkFechamento={linkFechamento} />
            ) : (
              <div className={`mt-4 rounded-2xl border border-yellow-400 p-5 ${
                temaPDF === "escuro" ? "bg-black text-white" : "bg-white text-zinc-950"
              }`}>
                <p className="text-[12px] font-black uppercase text-yellow-500">Garantias e suporte</p>
                <div className="mt-3 grid grid-cols-2 gap-3">
                  <Diferencial texto="Garantia dos equipamentos conforme o fabricante." tema={temaPDF} />
                  <Diferencial texto="Garantia do serviço conforme o escopo contratado." tema={temaPDF} />
                  <Diferencial texto="Orientação de uso na entrega do sistema." tema={temaPDF} />
                  <Diferencial texto="Suporte pós-venda da equipe CHOQUESEG." tema={temaPDF} />
                </div>
              </div>
            )}
        ...
        {totalPaginas === 3 && (
          <Pagina numeroPagina={3} totalPaginas={3} tema={temaPDF}>
            ...
            <div className={`mt-4 overflow-hidden rounded-2xl border ${
              temaPDF === "escuro" ? "border-zinc-700 bg-black" : "border-zinc-300 bg-white"
            }`}>
              <div className={`grid grid-cols-[minmax(0,1fr)_65px_80px_115px_115px] px-3 py-3 text-[11px] font-black uppercase ${
                temaPDF === "escuro" ? "bg-zinc-900 text-zinc-300" : "bg-zinc-200 text-zinc-700"
              }`}>
                <span>Item</span>
  ...
  return (
    <section className={`${compacto ? "mt-3" : "mt-4"} shrink-0`}>
      <div className="grid grid-cols-3 gap-3">
        <Resumo titulo="Subtotal" valor={moeda(Number(dados.subtotal ?? 0))} tema={dados.temaPDF} />
        <Resumo titulo="Desconto" valor={moeda(Number(dados.desconto ?? 0))} tema={dados.temaPDF} />
        <Resumo titulo="Valor à vista" valor={moeda(Number(dados.total ?? 0))} tema={dados.temaPDF} destaque />
      </div>

      <div className="mt-3 grid grid-cols-2 gap-3">
        <CondicaoPagamento
          titulo="Pagamento à vista"
          valor={moeda(Number(dados.total ?? 0))}
        ...
        <CondicaoPagamento
          ...
          valor={
            Number(dados.parcelasCartao ?? 0) > 1
              ? `${Number(dados.parcelasCartao ?? 0)}x de ${moeda(Number(dados.parcelaCartao ?? 0))}`
              : "Não informado"
          }
          descricao={
            Number(dados.parcelasCartao ?? 0) > 1
              ? `Total no cartão: ${moeda(Number(dados.totalCartao ?? 0))}`
