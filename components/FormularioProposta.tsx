...
export default function FormularioProposta() {
  ...
  useEffect(() => {
    ...
    if (salvo === "dividido" || salvo === "formulario" || salvo === "pdf") setModoVisualizacaoSolar(salvo);
  }, []);
 ...
 async function gerarPDF() {
  ...
  return (
    <main className="min-h-screen bg-zinc-950 text-white">
      <header className="sticky top-0 z-30 border-b border-yellow-400/40 bg-black/95 px-4 py-3 backdrop-blur md:px-6 xl:static xl:shrink-0">
      ...
      <div className="mx-auto w-full max-w-[1920px] shrink-0 px-3 pt-3 xl:px-4">
        <div className="grid grid-cols-3 gap-2 rounded-2xl border border-zinc-800 bg-black p-2">
          {[
            ...
            <button key={modo} type="button"
              onClick={() => alterarModoVisualizacaoSolar(modo as "dividido" | "formulario" | "pdf")}
              className={`rounded-xl px-2 py-3 text-xs font-black uppercase ${modoVisualizacaoSolar === modo ? "bg-yellow-400 text-black" : "bg-zinc-900 text-zinc-300"}`}>
              {nome}
            </button>
          ))}
        </div>
      ...
      <div className={`mx-auto w-full max-w-[1920px] gap-3 p-3 xl:p-4 ${
        modoVisualizacaoSolar === "dividido" ? "grid grid-cols-2" : "block"
      }`}>
        <aside
          className={`min-w-0 rounded-3xl border border-yellow-400/50 bg-black p-4 ${
            ...
            modoVisualizacaoSolar === "dividido"
              ? "h-[calc(100dvh-190px)] min-h-[520px] overflow-y-auto overscroll-contain"
              : "mx-auto h-[calc(100dvh-190px)] min-h-[520px] max-w-5xl overflow-y-auto overscroll-contain"
          }`}
        >
...
Cartão: {cartao}.`}
        ...
        <section
          ...
          className={`min-w-0 rounded-3xl border border-zinc-800 bg-zinc-900/60 p-2 ${
            ...
            modoVisualizacaoSolar === "dividido"
              ? "h-[calc(100dvh-190px)] min-h-[520px] overflow-auto overscroll-contain"
              : "mx-auto h-[calc(100dvh-190px)] min-h-[520px] max-w-[1500px] overflow-auto overscroll-contain"
          }`}
        >
          <div className="flex min-w-max w-full justify-center overflow-visible">
            <div
