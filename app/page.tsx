...
import RecibosModule from "@/modules/recibos/RecibosModule";
...
export default function Home() {
  ...
  const [tipoOrcamentoRapido, setTipoOrcamentoRapido] = useState<
    "seguranca-eletronica" | "eletrica" | "automacao"
  >("seguranca-eletronica");
  ...
  useEffect(() => {
    try {
      const salvo = localStorage.getItem("choqueseg-tema-sistema");
      if (salvo === "claro" || salvo === "escuro") setTemaSistema(salvo);
    } catch {}
  }, []);

  function alternarTemaSistema() {
    setTemaSistema((atual) => {
      const novo = atual === "escuro" ? "claro" : "escuro";
      try { localStorage.setItem("choqueseg-tema-sistema", novo); } catch {}
      return novo;
    });
  }

  function abrirOrcamentoRapido(
    tipo: "seguranca-eletronica" | "eletrica" | "automacao",
  ) {
    setTipoOrcamentoRapido(tipo);
    setTelaAtual("orcamento-rapido");
  }

  function navegarPeloTopo(tela: TelaSistema) {
    setTelaAtual(tela);
    setMenuTopoAberto(false);
  }

  function chaveCardsDashboard() {
    const usuario = usuarioLogado?.nome?.trim() || "usuario-local";
    return `choqueseg-dashboard-modulos:${usuario}`;
  }

  const MODULOS_FIXAVEIS: TelaSistema[] = [
    "clientes",
    "funil",
    "orcamento-rapido",
    "precificacao",
  ...
  function modulosDashboardAtuais(): TelaSistema[] {
    try {
      const salvo = localStorage.getItem(chaveCardsDashboard());
      if (!salvo) {
        return [
          "clientes",
          "funil",
      ...
      return Array.isArray(lista)
        ? lista.filter((item) => MODULOS_FIXAVEIS.includes(item))
        : [];
    } catch {
      return [];
    }
  ...
  function moduloEstaNoDashboard(tela: TelaSistema) {
    return modulosDashboardAtuais().includes(tela);
  }

  function alternarModuloNoDashboard(tela: TelaSistema) {
    if (!MODULOS_FIXAVEIS.includes(tela)) return;

    const atuais = modulosDashboardAtuais();
    const novos = atuais.includes(tela)
      ? atuais.filter((item) => item !== tela)
      : [...atuais, tela];
  ...
  return (
    <div className={`${temaSistema === "claro" ? "tema-claro bg-white text-zinc-950" : "tema-escuro bg-zinc-950 text-white"} min-h-screen`}>
      <style jsx global>{`
        .tema-claro {
        ...
        .tema-claro .bg-black,
        .tema-claro .bg-zinc-950,
        .tema-claro .bg-zinc-900,
        .tema-claro .bg-zinc-800 {
          background-color:#ffffff !important;
        }
        .tema-claro .bg-yellow-400\/5,
        .tema-claro .bg-yellow-400\/10 {
          background-color:#eff6ff !important;
        }
        .tema-claro .text-white,
        .tema-claro .text-zinc-100,
        .tema-claro .text-zinc-200,
        .tema-claro .text-zinc-300 {
          color:#0f172a !important;
        }
        .tema-claro .text-zinc-400,
        .tema-claro .text-zinc-500 {
          color:#475569 !important;
        }
        .tema-claro .text-yellow-300,
        .tema-claro .text-yellow-400,
        .tema-claro .text-yellow-500 {
          color:#1d4ed8 !important;
        }
        .tema-claro .border-yellow-400\/20,
        .tema-claro .border-yellow-400\/30,
        .tema-claro .border-yellow-400\/35,
        .tema-claro .border-yellow-400\/40,
        .tema-claro .border-yellow-400\/50,
        .tema-claro .border-yellow-400\/60,
        .tema-claro .border-yellow-400\/70,
        .tema-claro .border-zinc-700,
        .tema-claro .border-zinc-800 {
          border-color:#cbd5e1 !important;
        }
        .tema-claro .shadow-2xl {
        ...
        .tema-claro button:not(.manter-amarelo):hover {
          border-color:#2563eb !important;
        }
        .tema-claro .tema-card {
          background:#ffffff !important;
          border-color:#cbd5e1 !important;
          box-shadow:0 2px 8px rgba(15,23,42,.05);
        }
        .tema-claro .tema-card:hover {
          border-color:#2563eb !important;
          box-shadow:0 4px 14px rgba(37,99,235,.10);
        }
        .tema-claro .tema-valor {
          color:#1d4ed8 !important;
        }
        .tema-claro .tema-titulo {
          color:#0f172a !important;
        }
        .tema-claro .tema-topo {
          background:rgba(255,255,255,.97) !important;
          color:#0f172a !important;
          border-color:#cbd5e1 !important;
        }
        .tema-claro .tema-topo .marca-choqueseg {
          color:#0f172a !important;
        }
      `}</style>
      {telaAtual !== "dashboard" ? (
...
{telaAtual === "configuracoes" && podeConfiguracoes && (
  <ConfigEmpresaModule />
          ...
          {telaAtual === "orcamento-rapido" && podeOrcamentoRapido && (
            <OrcamentosRapidosModule tipo={tipoOrcamentoRapido} />
          )}
